...
<Tip>
  For one-off recoloring on a single device, viewers can open an event's detail modal and use the **Custom Color** action instead of editing `event_styles`. See [Custom event colors](/configuration/event-management#custom-event-colors) for details.
</Tip>
