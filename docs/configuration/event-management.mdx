...
<ParamField path="hide_event_actions" default="[]" type="list">
  Hide selected actions from the event detail popup. Supported values are `delete`, `custom_color`, `forward`, and `edit`. If every action is hidden, the modal's action footer is omitted entirely.
