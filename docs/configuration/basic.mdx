...
<ParamField path="title" default="Family Calendar" type="string">
  The title displayed in the card header. Set it to an empty string (`""`) to hide the title while keeping other header content — such as the header time, weather, controls, or custom `header_items` — visible. If nothing else is shown in the header, the entire header band is omitted.
  ...
  ```yaml
  # Hide the title but keep the rest of the header
  title: ""
  ```
</ParamField>
