...
### Week Compact day header

These options apply only to the **Week Compact** view. The spacing value controls both the space below the header and its bottom padding. The header continues to size naturally around dates, weather, and day badges.

<ParamField path="week_compact_weekday_font_size" default="12" type="number">
  Set the weekday-name font size in pixels.
</ParamField>

<ParamField path="week_compact_weekday_color" type="string">
  Set the weekday-name color to any CSS color value. Leave it unset to use the existing light, dark, or custom-background color.
</ParamField>

<ParamField path="week_compact_day_header_spacing" default="12" type="number">
  Set the day header's vertical spacing and bottom padding in pixels. Use a smaller value to shorten the header without imposing a fixed height.
</ParamField>

```yaml
type: custom:daylight-calendar-card
default_view: week-compact
week_compact_weekday_font_size: 10
week_compact_weekday_color: "#475569"
week_compact_day_header_spacing: 4
```
