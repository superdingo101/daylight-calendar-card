...
<ParamField path="display_full_weekday_names" default="false" type="boolean">
  Show full localized weekday names (for example, `Monday`) instead of the default abbreviated form (`Mon`). Applies to the Month, Week, Schedule, and Agenda views as well as the single-day event modal. Labels follow your Home Assistant language setting.
</ParamField>
