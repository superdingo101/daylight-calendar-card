---
title: "Appearance and Colors: Daylight Calendar Card"
...
<Note>
  The card inherits Home Assistant theme values for its background, border radius, border width, border color, and box shadow. If your active HA theme sets `ha-card-background`, `ha-card-border-radius`, `ha-card-border-width`, `ha-card-border-color`, or `ha-card-box-shadow`, the card picks them up automatically. The built‑in light/dark card color is only used as a final fallback when none of those theme values are defined.
</Note>
