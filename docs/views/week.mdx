---
title: "Week View: Glanceable Weekly Calendar Layout"
sidebarTitle: "Week"
description: "Week view shows each day of the week as a column with stacked events, providing a clean, glanceable layout for family dashboards and wall tablets."
---
...
<ParamField path="show_week_numbers_week" default="false" type="boolean">
  Used by **Week** only.

  Shows the ISO week number in the header. If the visible dates span multiple ISO weeks, the header shows the visible week range (for example, **CW 37–38**). A range that crosses the ISO year boundary is split instead of shown as a descending range (for example, **CW 52–53 / 1**).

  The prefix is localized automatically and can be customized or hidden with `week_number_prefix`.
</ParamField>
