...
<ParamField path="month_day_tap_action" default="create" type="string">
  ...
  - `create` — tapping a day opens the new-event form prefilled with that date (default).
  - `day_view` — tapping a day that has events opens a popup listing **all** of that day's events; tap any event there to view or edit it. Empty days still open the new-event form, so blank days stay quick to add to. When event management is enabled, the popup also shows an **Add Event** button for that day.
