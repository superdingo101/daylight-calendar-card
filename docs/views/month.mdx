...
<ParamField path="show_week_numbers_month" default="false" type="boolean">
  ...
  Adds an ISO week-number column on the left edge of the grid (e.g., **CW 23** in English, **wk 23** in Dutch). The prefix is localized automatically — use `week_number_prefix` to override it. Handy for work or project planning where teams reference week numbers.
</ParamField>

<ParamField path="week_number_prefix" default="null" type="string">
  Used by **Month** and **Week** when their week-number options are enabled.

  Controls the text shown before ISO week numbers in the Month grid and Week header. Three modes:

  - **Omit the option** (or set it to `null`) to use the localized default for the card's language (`CW` in English, `wk` in Dutch, `Sem` in French, and so on).
  - **Set a custom string** to replace the prefix. Surrounding whitespace is trimmed and the card adds exactly one space between the prefix and the number.
  - **Set an empty string** (`""`) to hide the prefix entirely and show only the week number.

  In the Month grid, the accessibility label always announces the full localized `Week {number}` phrase for screen readers regardless of the visible prefix.

  ```yaml
  type: custom:daylight-calendar-card
  show_week_numbers_month: true
  # Custom prefix renders "Week 23"
  week_number_prefix: Week
  ```

  ```yaml
  type: custom:daylight-calendar-card
  show_week_numbers_month: true
  # Number only renders "23"
  week_number_prefix: ""
  ```
</ParamField>
