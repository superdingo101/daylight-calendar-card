...
<ParamField path="month_day_tap_action" default="create" type="string">
  ...
  - **`create`** (default) — preserves the existing behavior:
    ...
    - otherwise, tapping a day that has events opens the day-events popup (see below);
    - otherwise (empty day, or no writable calendars) → nothing happens.
  - **`show_events`** — tapping a day that **has events** opens a popup listing **all** of that day's events; tap any event there to view, edit, or delete it. **Empty days still open the new-event form** (when event management is enabled and a writable calendar exists), so blank days stay quick to add to.

  In the `show_events` popup, an **Add Event** button (prefilled with that date) is shown when **all** of the following are true: event management is enabled, at least one writable calendar exists, and [`hide_add_event_button`](#hide_add_event_button) is `false`.
