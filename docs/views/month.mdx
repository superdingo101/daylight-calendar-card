...
<ParamField path="month_day_tap_action" default="create" type="string">
  ...
  - **`show_events`** — tapping a day that **has events** opens a popup listing **all** of that day's events; tap any event there to view or edit it (closing the event returns you to the day list). **Empty days still open the new-event form** (when event management is enabled and a writable calendar exists), so blank days stay quick to add to.
