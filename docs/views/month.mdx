...
<ParamField path="week_number_prefix" default="null" type="string">
  Used by **Month** only, and only when `show_week_numbers_month` is `true`.

  Controls the text shown before each ISO week number in the Month grid. Three modes:
  ...
  Regardless of the visible label, the accessibility label always announces the full localized `Week {number}` phrase for screen readers.
