...
<Tip>
  Compact Agenda titles wrap to a second line by default so long names never widen the day row. If you prefer one-line titles truncated with an ellipsis, add this `uix.style` override:
